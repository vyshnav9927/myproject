...
function wpmlm_custom_checkout_field_update_order_meta($order_id)
{
    ...
    remove_action('woocommerce_order_status_completed', 'wpmlm_woocommerce_order_status_completed',10);
 ...
 */
 ...
 function wpmlm_woocommerce_registration($order_id){
    ...
    if ($sponsor_username == '') {   
        ...
        if(isset($_SESSION['payment_status']) && $back_user_res){
        ...
        if (!$back_user_res) {
            ...
            if (!empty($user_details)) {
                ...
                if($order->status=='completed' || $order->status=='processing'){
                    ...
                    wpmlm_insert_unpaid_users($user_details);
 ...
 add_action('woocommerce_order_status_changed','wpmlm_woocommerce_registration',10,3);
...
// }
// add_action('woocommerce_thankyou', 'wpmlm_woocommerce_registration', 10, 1);

add_action( 'woocommerce_payment_complete','payment_status_complete',10,1);

function payment_status_complete($order_id){
...
/**
 * Inserting Level commission and self commission after
 * woocommerce status manaully changed into completed.
 * @param  [type] $order_id [description]
 * @return [type]           [description]
 */

function wpmlm_woocommerce_order_status_completed($order_id) { 
    ...
    $unpaid_user_res=wpmlm_get_unpaid_user($user_id);
    $package_id = $product_id[0];
        
    if($unpaid_user_res->user_id){
        
        // $unpaid_user=wpmlm_get_unpaid_user($user_id);
        $user_level   =    wpmlm_get_user_level_by_parent_id($unpaid_user_res->user_parent_id);
        $user_dob     =    get_post_meta( $order_id, 'reg_billing_date_of_birth',  true);
        $newDate      =    date("Y-m-d", strtotime($user_dob));
        $user_details=array(
            'user_ref_id' => $user_id,
            'user_parent_id' => $unpaid_user_res->user_parent_id,
            'user_first_name' => $unpaid_user_res->user_first_name,
            'user_second_name' => $unpaid_user_res->user_second_name,
            'user_address' => $unpaid_user_res->user_address,
            'user_city' => $unpaid_user_res->user_city,
            'user_state' => $unpaid_user_res->user_state,
            'user_country' => $unpaid_user_res->user_country,
            'user_zip' => $unpaid_user_res->user_zip,
            'user_mobile' => $unpaid_user_res->user_mobile,
            'user_email' => $unpaid_user_res->user_email,
            'user_dob' => $newDate,
            'user_level' => $user_level,
            'user_registration_type' => $unpaid_user_res->user_registration_type,
            'join_date' => date("Y-m-d H:i:s"),
            'user_status' => 1,
            'package_id' => $product_id[0]
        );
        ...
        if (wpmlm_insert_user_registration_details($user_details)) {
            ...
            wpmlm_insert_tran_password($tran_pass_details);
            wpmlm_insertBalanceAmount($user_id);  
            wpmlm_delete_unpaid_user($user_id);
...
add_action( 'woocommerce_order_status_completed', 'wpmlm_woocommerce_order_status_completed', 10, 1 );
...
function wpmlm_customized_woocommerce_menu($items)
{
    ...
    $is_unpaid_user=wpmlm_get_unpaid_user($user_data->ID);
    if ($user_status) {
        // $user_data = wp_get_current_user();
